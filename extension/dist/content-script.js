...
(function () {
    if (window.__fitted_extension_initialized)
        return;
    window.__fitted_extension_initialized = true;
    // 1. Hover Badge State & Injection
    let activeHoverBadge = null;
    ...
    function isProductCandidate(img) {
        ...
        if (rect.width < 140 || rect.height < 160)
            return false;
        if (img.closest("header, nav, footer, #fitted-extension-host"))
            return false;
    ...
    function attachProductListeners(img) {
        if (img.__fitted_scanned)
            return;
        img.__fitted_scanned = true;
        img.addEventListener("mouseenter", () => {
            if (!isProductCandidate(img))
                return;
            activeTargetImg = img;
            const rect = img.getBoundingClientRect();
            if (!activeHoverBadge) {
    ...
    let widgetHost = null;
    let shadowRoot = null;
    let currentGarmentUrl = "https://images.unsplash.com/photo-1544022613-e87ca75a784a?q=80&w=1000";
    let currentGarmentName = "Architectural Outerwear";
    // Widget runtime state
    let cameraStream = null;
    let isStreaming = false;
    let isMinimized = false;
    let capturedResultUrl = null;
    let capturedInputUrl = null;
    ...
    function setupWidgetInteractions(panel, root) {
        // 1. Drag logic
        ...
        root.getElementById("btn-capture").addEventListener("click", async () => {
            const canvas = root.getElementById("live-canvas");
            if (!canvas)
                return;
            ...
            const resultDisplay = root.getElementById("result-display");
    ...
    async function startWebcamInWidget(root) {
        const video = root.getElementById("live-video");
        const canvas = root.getElementById("live-canvas");
        if (!video || !canvas)
            return;
        try {
    ...
    function openWidgetWithGarment(garmentUrl, garmentName) {
        ensureWidgetCreated();
        ...
        if (shadowRoot) {
            ...
            if (label)
                label.textContent = `Garment: ${garmentName}`;
